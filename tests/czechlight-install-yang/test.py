    ...
    def _read_config(self):
        ...
        with open(self._config_file) as f_config:
            ...
            if 'module' in config_json:
                self._tested_module = config_json['module']
...
def compare(ctx, actual_file, expected_file):
    ...
    if ctx._tested_module:
        if ctx._tested_module not in actual:
            log("key '{}' not found in exported file".format(ctx._tested_module))
            return False

        if ctx._tested_module not in expected:
            log("key '{}' not found in expectedfile".format(ctx._tested_module))
            return False

        actual = actual[ctx._tested_module]
        expected = expected[ctx._tested_module]
