...
| 3 | Variable | Optical calibration data (size is given in field no. 2, size cannot be zero). Structure is described below. |
| 4 | 4 | CRC32 checksum spanning fields 0 to 3, the same CRC32 variant as ONIE's. |

### Optical calibration data

The calibration block is mandatory and must have non-zero length.
It always begins with a version number (currently fixed at `0x00`).
