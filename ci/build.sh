...
if [[ ${TRIGGERED_VIA_DEP} == 1 ]]; then
    ...
    for PROJECT in cla-sysrepo netconf-cli gammarus; do
        ...
        REBUILD_EXTRA_TARGETS="$REBUILD_EXTRA_TARGETS ${PROJECT}-reconfigure"
    done
fi
...
make -j${CI_PARALLEL_JOBS} --output-sync=target $REBUILD_EXTRA_TARGETS rootfs-czechlight-rauc
