...
CI_BUILDROOT_DIR=$(jq -r '.projects | map(select(.name == "github/buildroot/buildroot"))[0].src_dir' < ~/zuul-env.json)
if [[ "${CI_BUILDROOT_DIR}" == "null" ]]; then
    echo "CI tooling error: Cannot determine locations of buildroot main tree"
    exit 1
fi
...
sed -i 's/^\(\.NOTPARALLEL\).*/#\1/' ${HOME}/${CI_BUILDROOT_DIR}/Makefile
